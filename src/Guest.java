...
public class Guest extends Cititor {
    ...
    public Guest(String prenume, String nume, int idCititor, List<Carte> cartiImprumutate) {
        super(prenume, nume, idCititor, cartiImprumutate);
    }

    public void imprumuta (Carte carte) {
        if(getCartiImprumutate().size() >= 2) {
            System.out.println("A fost atinsă limita de cărți împrumutate pentru cititor guest. Returnează una din " +
            ...
            System.out.println("Ai împrumutat încă o carte. Mai poți împrumuta " + (2 - cartiActuale.size()) +
                    " cărți.");
    ...
    public void returneaza (Carte carte) {
        if(!getCartiImprumutate().contains(carte)) {
            ...
            setCartiImprumutate(cartiActuale);
            System.out.println("A fost returnată o carte. Cititorul mai are " + cartiActuale.size() + " carți " +
                    "împrumutate.");
            carte.setNumarExemplare(carte.getNumarExemplare() + 1);
        }
    }
